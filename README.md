...
Omni is a Python-based autonomous Minecraft agent that connects to a Minecraft server, observes the world, nearby entities, and its own runtime state, uses a local LLM for high-level reasoning, and executes actions through a deterministic task and tool layer.
...
- Minecraft observations include `vision`, `inventory`, and structured `entities`
- Nearby entities are split into:
  - living nearby entities
  - dropped items on the ground
  - vehicles such as boats and minecarts
- Entity observations include summaries and basic attitude grouping such as `hostile`, `neutral`, `friendly`, and `utility`
- Entities are intentionally observation-only for now and are not persisted in `WorldState`
...
- Direct entity interaction still does not exist; current entity support is perception-only
